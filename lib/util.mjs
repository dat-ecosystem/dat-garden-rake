...
export function plusMinus (number, plus, minus) {
  if (minus === undefined) {
    minus = plus
  }
  return number * (1.0 - minus) + number * (plus + minus) * Math.random()
}

export function plusMinusInt (number, plus, minus) {
  return Math.round(plusMinus(number, plus, minus))
}
...
export function taskProcessor ({ type, getTaskDef, exec, validateTask }) {
  ...
  return {
    ...
    async createTasks (api, items) {
      // TODO: use runtime mutex-locks to make sure that parallel requestsd don't create
      // duplicate tasks here!
