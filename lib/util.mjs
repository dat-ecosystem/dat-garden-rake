...
export function taskProcessor (type, getTaskDef, exec) {
  return {
    type,
    getTaskDef,
    async createTasks (api, items) {
      return await createTasks(api, items.map(item => getTaskDef(api, type, item)))
    },
    async createTask (api, item) {
      return await createTasks(api, [getTaskDef(api, type, item)])
    },
...
export function resourceTaskProcessor (type, getDB, getTaskDef, create) {
  return {
    type,
    getDB,
    getTaskDef,
    async createTasks (api, items) {
      return await createResourceTasks(api, getDB(api), items.map(item => getTaskDef(api, type, item)))
    },
    async createTask (api, item) {
      return await createResourceTasks(api, getDB(api), [getTaskDef(api, type, item)])
    },
...
async function createTasks (api, taskDefs) {
...
async function createResourceTasks (api, db, taskDefs) {
  ...
  return await createTasks(api, taskDefs)
}
